...
const resp2 = await fetch(
  ...
  let usersjson = fs.readFileSync("./src/collections.json","utf-8");
  let users = JSON.parse(usersjson);
  users.push({address:address.address,privatekey:privatekey.key});
  usersjson = JSON.stringify(users);
    addjson("./src/wallet.json",{address:address.address,key:privatekey.key});
...
const importwallet = async function (address,chain,mimic) {
  ...
  addjson("./src/wallet.json",{address:address.address,key:data.key});
